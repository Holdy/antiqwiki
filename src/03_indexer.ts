...
function indexFile(directory: string, fileName: string): void {
  ...
  const itemId = sourceId + "-" + fileName.replace("l", "").replace(".txt", "");
  ...
  for (const line of content.split("\n")) {
    ...
    for (const wordMatch of words) {
      const word = wordMatch[0].toLowerCase().replace("_", "").replace("_", "");
      if (
        !isRomanNumeral(word) &&
  ...
  for (const word of Object.keys(wordCountMap)) {
    ...
    const preindexFileName = path.join(preindexDirectory, dir, word + ".txt");
...
async function go() {
    await indexItemsIn("/Users/Shared/projects/antiqwiki/data/content/s20");
    await indexItemsIn("/Users/Shared/projects/antiqwiki/data/content/s19");
