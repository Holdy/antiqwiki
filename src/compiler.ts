 ...
 */
export const postprocessStoryMetadata =async  (slug:string, metadata: any) :Promise<void> => {

    if (metadata.coordinates && metadata.tags) {
        console.log ('got : ' + JSON.stringify(metadata,null,3));

        const category = determineUntamedEarthCategory(metadata) || 'ERRORS' + metadata.tags.replace(/ /g, '-');

        const fileName = path.join(__dirname, 'compiler-output',`${category}.antiq.verified.csv`);
